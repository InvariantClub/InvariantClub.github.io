...
main = do
  ...
  hakyllWith config $ do
    ...
    match "guides/**.md" $ do
      route $ setExtension "html"
      tags <- buildTagsWith getTags "guides/**" (fromCapture "tags/*.html")
      let ctx =  constField "commit"  commitDetails
              <> postContext
    ...
    match "guides.html" $ do
      route idRoute
      compile $ do
        guides <- recentFirst =<< loadAll "guides/**"

        let ctx =  constField "commit" commitDetails
                <> listField  "guides" postContext (pure guides)
                <> constField "title" "Blog"
                <> postContext
...
context =
  constField "rootUrl" "https://invariant.club"
